...
select
  ...
  (select substring(name,1,4) from collection where collection_id=i.owning_collection) col_owner_name4,
  array_to_string(array(
      select substring(name,1,4) from collection2item c2i2, collection c2 where c2i2.item_id=i.item_id and
  ...
  array_to_string(array(
    select substring(text_value,1,4) from metadatavalue where item_id=i.item_id and metadata_field_id=
      (select metadata_field_id from metadatafieldregistry where element='subject' and qualifier='forgroup')
  ), '||') forcodes,
  array_to_string(array(
    select regexp_replace(text_value, '[[:space:]]+', ' ', 'g') from metadatavalue where item_id=i.item_id and metadata_field_id=
      (select metadata_field_id from metadatafieldregistry where element='subject' and qualifier='forgroup')
  ), '||') forgroups,
...
with
  ...
  csv
    header
    force quote col_owner_name4, col_others_name4
"

descr="Show the RMID, item-handle, owner-handle, list of
collection-handles, forcodes, forgroups and miscellaneous
