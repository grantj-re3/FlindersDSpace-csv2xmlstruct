...
select
  ...
  (select count(*) from metadatavalue where item_id=i.item_id and metadata_field_id=
    (select metadata_field_id from metadatafieldregistry where element='type' and qualifier is null)
  ) dc_type_count,
  i.item_id,
...
with
  ...
  csv
    header
    force quote rmid, dc_type
"

descr="Show the RMID, dc_type and miscellaneous information for each item:
